...
async def menu_handler(call: CallbackQuery):
    '''Select category'''
    await call.message.edit_text(text="Выберите категорию")
...
async def order_user_handler(call: CallbackQuery, state: FSMContext):
    '''Select category'''
    await call.message.edit_text(text="Список заказов")
    data = await state.get_data()
    if data:
        ...
        await call.message.answer(text="Подтвердить заказ",
                                  reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(
                                      text="Подтвердить заказ",
                                      callback_data="accept"
                                  )]]))
        await call.message.answer(text="Назад", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(
            text="Назад",
...
async def dish_handler(call: CallbackQuery):
    ...
    await call.message.edit_text(text="Выберите блюдо")
    for val in dishes:
        if val.dish_photo:
